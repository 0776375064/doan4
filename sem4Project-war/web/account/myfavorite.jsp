...
<div class="section-chart">
    ...
    <div class="container">
        <div class="section-top">My favorite songs</div>
        <div class="row">
                            ...
                            <tr>
                                ...
                                <td>
                                    <c:if test="${sessionScope.accountID != null}">
                                        <c:if test="${controller.songInfo(day).price != 0}">
                                            <c:if test='${controller.alreadyBought(sessionScope.accountID,controller.songInfo(day).songID) == true}'>
                                                <a href="" data-id="${controller.songInfo(day).songID}" class="btn-link btn-play"><i class='bx bx-play'></i>Play</a>
                                            </c:if>
                                            <c:if test='${controller.alreadyBought(sessionScope.accountID,controller.songInfo(day).songID) == false}'>
                                                <a href="SongDetail?songID=${controller.songInfo(day).songID}" class="btn-link btn-order">Buy ${controller.songInfo(day).price} $</a>
                                            </c:if>
                                        </c:if>
                                        <c:if test="${controller.songInfo(day).price == 0}">
                                            <a href="" data-id="${controller.songInfo(day).songID}" class="btn-link btn-play"><i class='bx bx-play'></i>Play</a>
